...
pal <- wes.palette(name = 'Cavalcanti', type = 'continuous')
num.cols <- 5
...
<<hill, eval = T, echo = F, results = 'hide', cache = T>>=
load('data/salwt_grd.RData')
load('data/epc_est.RData')
...
% figure used on title page
<<title_fig2, echo = F, results = 'hide', message = F, cache = T>>=
load('data/salwt_grd.RData')
load('data/epc_est.RData')
...
pdf('fig/title_plo2.pdf', width = 8, height = 2.75, family = 'serif')
print(p1)
dev.off()
@
...
sites <- c('APACP', 'GNDBL', 'MARMB', 'RKBMB', 'WKBWB')
# for(site in sites){
#   load(paste0('M:/wq_models/SWMP/raw/rproc/proc5/', site, '.RData'))
#   dat <- get(site)
#   dat$year <- strftime(dat$DateTimeStamp, '%Y')
#   dat <- dat[dat$year %in% '2012', ]
#   
#   assign(site, dat)
#   save(list = site, file = paste0('data/', site, '.RData'))
# }
#   
